...
async function starts() {
	const reinoob = new WAConnection()
	reinoob.logger.level = 'warn'
	console.log(banner.string)
	reinoob.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scan the qr code above'))
	})

	fs.existsSync('./BarBar.json') && reinoob.loadAuthInfo('./BarBar.json')
	reinoob.on('connecting', () => {
		start('2', 'Connecting...')
	})
	reinoob.on('open', () => {
		success('2', 'Connected')
	})
	await reinoob.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./BarBar.json', JSON.stringify(reinoob.base64EncodedAuthInfo(), null, '\t'))

	reinoob.on('group-participants-update', async (anu) => {
		const mdata = await reinoob.groupMetadata(anu.jid)
		if(antifake.includes(anu.jid)) {
			if (anu.action == 'add'){
				num = anu.participants[0]
				if(!num.split('@')[0].startsWith(55)) {
					reinoob.sendMessage(mdata.id, 'Corre, o travador chegou', MessageType.text)
					setTimeout(async function () {
						reinoob.groupRemove(mdata.id, [num])
					}, 1000)
				}
			}
		}
		...
		try {
            ...
            try {
                var pp_user = await reinoob.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
            } catch (e) {
                var pp_user = 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60'
            }
            if (anu.action == 'add') {
            	num = anu.participants[0]
                group_info = await reinoob.groupMetadata(anu.jid)
                welkam = `Bem Vindo @${num.split('@')[0]}`
                reinoob.sendMessage(anu.jid, anu_img, MessageType.image, { caption: welkam })
            } else if (anu.action == 'remove') {
            	num = anu.participants[0]
                anu_img = await getBuffer(`http://api.lolhuman.xyz/api/welcomeimage?apikey=${apilol}&img=${pp_user}&text=Migui%20Bot`)
                group_info = await reinoob.groupMetadata(anu.jid)
                out = `1 minuto de silêncio @${num.split('@')[0]}`
                reinoob.sendMessage(anu.jid, anu_img, MessageType.image, { caption: out })
            }
		} catch (e) {
	...
	reinoob.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: 'Que isso mana ? você não é o reinoob',				
					admin: '❌ Você não é adm! ❌',
					Badmin: '❌ Preciso ser adm! ❌'
				}
			}

			const botNumber = reinoob.user.jid
			const ownerNumber = [`${setting.ownerNumber}@s.whatsapp.net`]
			const isGroup = from.endsWith('@g.us')
			const tescuk = ["0@s.whatsapp.net"]
			const sender = isGroup ? mek.participant : mek.key.remoteJid
			const groupMetadata = isGroup ? await reinoob.groupMetadata(from) : ''
			...
			pushname = reinoob.contacts[sender] != undefined ? reinoob.contacts[sender].vname || reinoob.contacts[sender].notify : undefined
			const isUrl = (url) => {
			    return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
			}
			const reply = (teks) => {
				reinoob.sendMessage(from, teks, text, {quoted:mek})
			}
			const sendMess = (hehe, teks) => {
				reinoob.sendMessage(hehe, teks, text)
			}
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? reinoob.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : reinoob.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
            const costum = (pesan, tipe, target, target2) => {
		     	reinoob.sendMessage(from, pesan, tipe, {quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target2}` }}})
			}
			
			if (messagesC.includes("#kick")){
				reinoob.updatePresence(from, Presence.composing)
				tujuh = fs.readFileSync('./assets/ban.mp3');
               reinoob.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
			}			
			 ...
			 //function antilink 
				if (messagesC.includes("://chat.whatsapp.com/")){
					...
					var kic = `${sender.split("@")[0]}@s.whatsapp.net`
						...
						reinoob.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
					}
			...
			if ((isAntiPorn && isBotGroupAdmins)) {
				...
				if (type === MessageType.image) {
					savedFilename = await reinoob.downloadAndSaveMediaMessage (mek)
					imgbbUploader(apikeyimgbb, savedFilename).then(async function(response) {
						...
						if(anu[0].className === 'Porn' && isGroupAdmins)  {
							await reinoob.sendMessage(from,adminmsgporn, MessageType.text, {quoted: mek})
							return
							
						} else if(anu[0].className === 'Porn' && !isGroupAdmins) {
							await reinoob.sendMessage(from,banmsgporn, MessageType.text, {quoted: mek})
							setTimeout(async function () {
								reinoob.groupRemove(from, [sender])
							}, 2000)
							return
						}
						...
						if(anu[0].className === 'Sexy') return reinoob.sendMessage(from,'Cuidado com oq manda em amigo, to com antiporn ativado', MessageType.text, {quoted: mek})
			...
			let authorname = reinoob.contacts[from] != undefined ? reinoob.contacts[from].vname || reinoob.contacts[from].notify : undefined	
...
if (!isGroup) return reply(mess.only.group)
menuimg = fs.readFileSync('./assets/menuimg.jpg')
reinoob.sendMessage(from, menuimg, image, {quoted: mek, caption: idioma(prefix), text})
lima = fs.readFileSync('./assets/id.mp3');
reinoob.sendMessage(from, lima, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'music':
if (!isGroup) return reply(mess.only.group)
reinoob.sendMessage(from, music(prefix, sender), text, { quoted: mek })
break
case 'biblioteca':
if (!isGroup) return reply(mess.only.group)
reinoob.sendMessage(from, biblioteca(prefix, sender), text, { quoted: mek })
break
				case 'produtos':
					if (!isGroup) return reply(mess.only.group)
						reinoob.sendMessage(from, produtos(prefix, sender), text, { quoted: mek })
						break
				case 'adms':
					if (!isGroup) return reply(mess.only.group)
						reinoob.sendMessage(from, adms(prefix, sender), text, { quoted: mek })
						break
				case 'fga':
					if (!isGroup) return reply(mess.only.group)
						reinoob.sendMessage(from, fga(prefix, sender), text, { quoted: mek })
						break
				case 'maker':
					if (!isGroup) return reply(mess.only.group)
						reinoob.sendMessage(from, maker(prefix, sender), text, { quoted: mek })
						break
				case 'entretenimento':
				    if (!isGroup) return reply(mess.only.group)
						reinoob.sendMessage(from, entretenimento(prefix, sender), text, { quoted: mek })
						break
				case 'about':
					if (!isGroup) return reply(mess.only.group)
						reinoob.sendMessage(from, about(prefix, sender), text, { quoted: mek })
						break
				case 'tipografia':
					if (!isGroup) return reply(mess.only.group)
						reinoob.sendMessage(from, tipo(prefix, sender), text, { quoted: mek })
						break
				case 'consultas':
					if (!isGroup) return reply(mess.only.group)
						reinoob.sendMessage(from, consultas(prefix, sender), text, { quoted: mek })
						break
				case 'credits':
				    if (!isGroup) return reply(mess.only.group)
						reinoob.sendMessage(from, credits(prefix, sender), text, { quoted: mek })
						break
				case 'flix':
					if (!isGroup) return reply(mess.only.group)
						reinoob.sendMessage(from, flix(prefix, sender), text, { quoted: mek })
						break
				case 'info':
					me = reinoob.user
					...
					reinoob.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
				case 'blocklist':
					...
					reinoob.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": blocked}})
					break
				case 'igdl':
                    ...
                    await reinoob.sendMessage(from, ini_buffer, ini_type, { quoted: mek })
                    break
                case 'twtdl':
                    ...
                    ini_buffer = await getBuffer(ini_url)
                    await reinoob.sendMessage(from, ini_buffer, video, { quoted: mek })
                    break
                 case 'fbdl':
                ...
                case 'ytplay':
                    ...
                    await reinoob.sendMessage(from, ini_buffer, image, { quoted: mek, caption: ini_txt })
                    get_audio = await getBuffer(get_result.audio[3].link)
                    await reinoob.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_info.title}.mp3`, quoted: mek })
                    get_video = await getBuffer(get_result.video[0].link)
                    await reinoob.sendMessage(from, get_video, video, { mimetype: 'video/mp4', filename: `${get_info.title}.mp4`, quoted: mek })
                    break
				case 's':
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						const media = await reinoob.downloadAndSaveMediaMessage(encmedia)
						ran = getRandom('.webp')
						await ffmpeg(`./${media}`)
							.input(media)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ${addMetadata('MiguiBOT', authorname)} ${ran} -o ${ran}`, async (error) => {
									...
									reinoob.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
									fs.unlinkSync(media)	
									fs.unlinkSync(ran)	
								})
								/*reinoob.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								...
								exec(`webpmux -set exif ${addMetadata('BOT', authorname)} ${ranw} -o ${ranw}`, async (error) => {
									if (error) return reply(mess.error.stick)
									reinoob.sendMessage(from, fs.readFileSync(ranw), sticker, {quoted: mek})
									fs.unlinkSync(ranw)
								})
								//reinoob.sendMessage(from, fs.readFileSync(ranw), sticker, {quoted: mek})
							})
				...
				case 'sticker2':
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
                        filePath = await reinoob.downloadAndSaveMediaMessage(encmedia)
                        ...
                        request({
                            ...
                            ffmpeg(`./${file_name}`)
                                ...
                                .on('end', function() {
                                    reinoob.sendMessage(from, fs.readFileSync(file_name2), sticker, { quoted: mek })
                ...
                case 'yt':
					...
					reinoob.sendMessage(from, thumb, image, {quoted: mek, caption: teks})
					buffer = await getBuffer(anu.result)
					reinoob.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.title}`, quoted: mek})
					break
			    case 'ocr':
                    if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
                        var encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
                        var filePath = await reinoob.downloadAndSaveMediaMessage(encmedia, filename = getRandom());
...
buffer = await getBuffer(anu.result)
reinoob.sendMessage(from, buffer, image, {quoted: mek, caption: 'Sapecagens 😈'})
break
case 'happymod':
if (!isGroup) return reply(mess.only.group)
...
reinoob.sendMessage(from, buffer, image, {quoted: mek, caption: `${teks}`})
...
reinoob.sendMessage(from, buffer, sticker, {quoted: mek})
fs.unlinkSync(rano)
})
await limitAdd(sender)
break
...
buffer = await getBuffer(anu.result)
reinoob.sendMessage(from, buffer, image, {quoted: mek, caption: 'Policiaaaaaaa 🚓'})
break
case 'add':
try {
if (!isGroup) return reply(mess.only.group)
...
reinoob.sendMessage(from, ct, image, {caption: 'Migui Bot🐒', quoted: mek})
break
case 'freefire':
if (args.length == 0) return reply(`Exemplo: ${prefix + command} reinoob`)
ini_txt = args.join(" ")
getBuffer(`https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${apilol}&text=${ini_txt}`).then((gambar) => {
reinoob.sendMessage(from, gambar, image, { quoted: mek })
})
break
case 'map':
if(!isGroup) return reply(mess.only.group)
...
reinoob.sendMessage(from, buffer, image, {quoted: mek, caption: `${body.slice(5)}`})
await limitAdd(sender)
break
case 'translate':
...
ini_url = ini_url.result
ini_buffer = await getBuffer(ini_url)
await reinoob.sendMessage(from, ini_buffer, image, { quoted: mek })
break
case 'roletahard':
if(!isGroup) return reply(mess.only.group)
if(!isBotGroupAdmins) return reply(mess.only.Badmin)
...
await reinoob.sendMessage(from, `💥O @${mem.split('@')[0]} Não teve sorte💥\n*😈 Agora se prepare para as consequências 😈*`, text, {contextInfo: {"mentionedJid": [mem]}})
await reinoob.groupRemove(from, [mem])
await reinoob.sendMessage(from, `👻O @${mem.split('@')[0]} teve sua vida ceifada👻`)
}, 10000)
break
case 'antiporn':
try {
...
break
				case 'tts':
					if (args.length < 1) return reinoob.sendMessage(from, 'Qual é o código da linguagem ?', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return reinoob.sendMessage(from, 'Cadê o texto', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							reinoob.sendMessage(from, buff, audio, {quoted: mek, ptt:true})
							fs.unlinkSync(rano)
						})
					})
					break
...
media = await reinoob.downloadAndSaveMediaMessage(encmedia)
ran = getRandom('.mp4')
exec(`ffmpeg -i ${media} ${ran}`, (err) => {
fs.unlinkSync(media)
if (err) return reply('❌ Falha ao converter vídeo para mp3 ❌')
buffer = fs.readFileSync(ran)
reinoob.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', quoted: mek})
fs.unlinkSync(ran)
})
break
case 'setprefix':
...
reinoob.sendMessage(from, ini_buffer, sticker)
}
break
case 'dado':
if (!isGroup) return reply(mess.only.group)
...
break
		case 'semoji':
                    ...
                    reinoob.sendMessage(from, ini_buffer, sticker, { quoted: mek})
                    break
                                case 'tagall2':
					members_id = []
					teks = (args.length > 1) ? body.slice(8).trim() : ''
					...
					reinoob.sendMessage(from, teks, text, {detectLinks: false, quoted: mek})
					break
				case 'clearall':
					if (!isOwner) return reply('Você não é o reinoob')
					anu = await reinoob.chats.all()
					reinoob.setMaxListeners(25)
					for (let _ of anu) {
						reinoob.deleteChat(_.jid)
					}
					reply('Chats limpos')
					break
					case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						buff = await reinoob.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							reinoob.sendMessage(_.jid, buff, image, {caption: `[reinoob falando]\n\n${body.slice(4)}`})
						}
						reply('Feito')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `「reinoob Falando」\n\n${body.slice(4)}`)
						}
						reply('Feito')
					}
					break
...
reinoob.groupRemove(from, mentioned)
} else {
mentions(`Sucesso: @${mentioned[0].split('@')[0]} Promovido para adm!`, mentioned, true)
reinoob.groupMakeAdmin(from, mentioned)
}
break
case 'adtip':
if (!isGroupAdmins) return reply(mess.only.admin)
...
delb = await reinoob.downloadMediaMessage(boij)
fs.writeFileSync(`./banco/tipo/${svst}.mp4`, delb)
reinoob.sendMessage(from, `Tipografia salva!`, MessageType.text, { quoted: mek })
break
case 'lv1':
...
reinoob.sendMessage(from, dua, document, {mimetype: Mimetype.pdf})
await limitAdd(sender)
})
break
case 'lv2':
...
reinoob.sendMessage(from, dua, video, {quoted: mek, caption: 'DJ Migui'})
await limitAdd(sender)
})
break
case 'macaco':
...
reinoob.sendMessage(from, dua, sticker, {quoted: mek})
await limitAdd(sender)
})
break
case 'random':
...
if (!isGroupAdmins) return reply(mess.only.admin)
teks = body.slice(6)
group = await reinoob.groupMetadata(from);
...
await reinoob.sendMessage(from, options, text)
break
case 'pedra':
let pedra = Math.floor(Math.random() * 3) + 1
if(pedra == '1'){
reinoob.sendMessage(from, `Papel, ganhei.`, text, {quoted: mek})
}
else if (pedra == '2') {
reinoob.sendMessage(from, `Tesoura;-; af, perdi`, text, {quoted: mek})
}
else{
reinoob.sendMessage(from, `Pedra tbm, oporra`, text, {quoted: mek})
}                   
...
if(papel == '1'){
reinoob.sendMessage(from, `Tesoura, ganhei.`, text, {quoted: mek})
}
else if (papel == '2') {
reinoob.sendMessage(from, `Pedra;-; af, perdi`, text, {quoted: mek})
}
else{
reinoob.sendMessage(from, `Papel tbm, oporra`, text, {quoted: mek})
}                   
...
if(tesoura == '1'){
reinoob.sendMessage(from, `Pedra, ganhei.`, text, {quoted: mek})
}
else if (tesoura == '2') {
reinoob.sendMessage(from, `Papel;-; af, perdi`, text, {quoted: mek})
}
else{
reinoob.sendMessage(from, `Tesoura tbm, oporra`, text, {quoted: mek})
}                   
...
reinoob.sendMessage(from, `Sticker salvo!`, MessageType.text, { quoted: mek })
break
case 'pinterest':
if (!isGroup) return reply(mess.only.group)
tels = body.slice(11)
...
reinoob.sendMessage(from, pok, image, { quoted: mek, caption: `*Migui 📌*\n\*Resultado da pesquisa* : *${tels}*`})
await limitAdd(sender)
break
                 case 'trigger':
					...
					reply(mess.wait)
					owgi = await  reinoob.downloadAndSaveMediaMessage(ger)
					...
					fs.unlinkSync(ranp)
					if (err) return reply(mess.error.stick)
					exec(`webpmux -set exif ${addMetadata('Migui', 'Bot')} ${rano} -o ${rano}`, async (error) => {
					if (error) return reply(mess.error.stick)
					reinoob.sendMessage(from, fs.readFileSync(rano), sticker, {quoted: mek})
					fs.unlinkSync(rano)
					})
					})
					} else {
				...
				case 'gay':		
					...
					reinoob.sendMessage(from, 'O quão gay você é: *'+rate+'*\n\nSua porcentagem gay : '+ kl+'%', text, { quoted: mek })
					break
             case 'addrandom':
				if (!isOwner) return reply(mess.only.ownerB)
			    if (!isQuotedSticker) return reply('Marca um sticker')
...
reinoob.sendMessage(from, kinycnpj, text, {quoted: mek})
await limitAdd(sender)
break
...
reinoob.sendMessage(from, box, text, {quoted: mek})
break					
...
reinoob.sendMessage(from, ''+hasil+'', text, {quoted: mek})
break					
...
reinoob.sendMessage(from, buffer, image, {quoted: mek})
break
...
await reinoob.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, quoted: mek })
break
...
if(!mentioned||mentioned.length < 1||mentioned.length > 1) return reinoob.sendMessage(from, 'Você precisa marcar alguém para esse comando', text, {quoted: mek})
reinoob.sendMessage(from, dua, video, {mimetype: Mimetype.gif, caption: `${pushname} ESTÁ TE AMEAÇANDO "o cú ou a vida?" ${body.split(' ').slice(1).join(' ')}`,quoted: mek, contextInfo: {"mentionedJid": mentioned}})                   
})
break
...
var mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
if(!mentioned||mentioned.length < 1||mentioned.length > 1) return reinoob.sendMessage(from, 'Você precisa marcar alguém para esse comando', text, {quoted: mek})
reinoob.sendMessage(from, dua, video, {mimetype: Mimetype.gif, caption: `${pushname} DEU UM TIRO NO(A) ${body.split(' ').slice(1).join(' ')}`,quoted: mek, contextInfo: {"mentionedJid": mentioned}})                   
})
break					
...
var mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
if(!mentioned||mentioned.length < 1||mentioned.length > 1) return reinoob.sendMessage(from, 'Você precisa marcar alguém para esse comando', text, {quoted: mek})
reinoob.sendMessage(from, dua, video, {mimetype: Mimetype.gif, caption: `KKKKK ${pushname} DEU MÓ TAPÃO NO(A) ${body.split(' ').slice(1).join(' ')}`,quoted: mek, contextInfo: {"mentionedJid": mentioned}})                   
})
break					
...
if(body.length<11) return reinoob.sendMessage(from, 'Onde está o texto?\n\n*Exemplo: ${prefix + command} reinoob*', text, {quoted: mek})
reinoob.sendMessage(from, ('Estou fazendo Aguarde...'), text, {quoted: mek})
...
slo = await reinoob.downloadAndSaveMediaMessage(low)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${slo} -filter:a "atempo=0.7,asetrate=44100" ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(slo)
if (err) return reply('Error!')
hah = fs.readFileSync(ran)
reinoob.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
fs.unlinkSync(ran)
})
break	
...
bas = await reinoob.downloadAndSaveMediaMessage(ass)
ran = getRandom('.mp3')
...
reinoob.sendMessage(from, nye, image, { caption: 'kkkkkkkkkkkk', quoted: mek })
break
case 'roleta':
let roleta = Math.floor(Math.random() * 2) + 1
if(roleta == '1'){
reinoob.sendMessage(from, 'ATIREI EM VOCÊ', text, {quoted: mek})
}
else{
reinoob.sendMessage(from, 'NÂO ATIREI', text, {quoted: mek})
}
...
reinoob.groupSettingChange(from, GroupSettingChange.messageSend, true);
reply(close)
break
case 'tiktok':
...
reinoob.sendMessage(from, mess.wait, text,{quoted : mek})
pyu = `✅ PRONTO! ✅
...
reinoob.sendMessage(from, buffer, video, {mimetype: 'video/mp4', filename: `${anu.no_watermark}.mp4`, quoted: mek, caption: pyu})
reinoob.sendMessage(from, buff, audio, {quoted : mek})
break
case 'on':
let ido = args && /\d+\-\d+@g.us/.test(args[0]) ? args[0] : from
let online = [...Object.keys(reinoob.chats.get(ido).presences), reinoob.user.jid]
reinoob.sendMessage(from, 'Monkeys Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join`\n`, text, { quoted: mek,
...
break
						case 'abrir':
							...
							reinoob.groupSettingChange(from, GroupSettingChange.messageSend, false)
							reinoob.sendMessage(from, open, text, { quoted: mek })
							break
							case 'play':
								if (!isGroup) return reply(mess.only.group)
									if(body.length < 5) return reinoob.reply(from, 'Você precisa dizer a música', mek)
									...
									reinoob.reply(from, `O limite de tamanho é 16 MB. Esse áudio possui ${asize.filesize}`, mek)
									}
									else{
									thumb = await getBuffer(res.thumbnail)
									reinoob.sendMessage(from, thumb, image, {quoted: mek, caption: 'Calma ae, tô procurando o CD'})
									...
									reinoob.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', quoted: mek, ptt: true})
									}
									break
				case 'play2':  
					...
					buffer = await getBuffer(anu.result.thumbnail)
				     ...
				     reinoob.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
					reinoob.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', filename: `${anu.title}.mp3`, quoted: mek})
					  break
				case 'demitir':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					...
					if (mentioned.length > 1) {
						...
						reinoob.groupDemoteAdmin(from, mentioned)
					}
					break
					case 'delete':
						case 'del':
						if (!isGroup) return reply(mess.only.group)
							if (!isGroupAdmins) return reply(mess.only.admin)
							reinoob.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
							break
							case 'hub':
								daddy = `${body.slice(4)}`
								no = daddy.split("/")[0];
								...
								reinoob.sendMessage(from, buffer, image, {quoted: mek, caption: 'Pronto camaco'})
								 await limitAdd(sender) 
								break
...
break
				...
				case 'link':  
                    ...
                    if (!isGroupAdmins) return reply(mess.only.admin)
                    if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                    linkgc = await reinoob.groupInviteCode(from)
                    reply('https://chat.whatsapp.com/'+linkgc)
                    break
                case 'sair':
                    if (!isGroup) return reply(mess.only.group)
                    if (isGroupAdmins || isOwner) {
                    	reinoob.groupLeave(from)
                    } else {
				...
				case 'img':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						buffer = fs.readFileSync(ran)
						reinoob.sendMessage(from, buffer, image, {quoted: mek, caption: 'kkkkkkkkkkkkkk'})
						fs.unlinkSync(ran)
					})
					break
case 'ia':
...
if (data.error) return reply(data.error)
kiny = `${data.result}`
reinoob.sendMessage(from, kiny, text, {quoted: mek})
await limitAdd(sender)
break
...
break
				case 'buscar':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						reply(mess.wait)
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						media = await reinoob.downloadMediaMessage(encmedia)
						await wait(media).then(res => {
							reinoob.sendMessage(from, res.video, video, {quoted: mek, caption: res.teks.trim()})
						}).catch(err => {
							reply(err)
						})
					} else {
